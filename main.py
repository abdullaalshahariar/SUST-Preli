...
def classify_case_type_from_history(ticket):
    """
    Case type derived ONLY from transaction history.
    """
...
def evaluate_evidence_verdict(ticket, trx, signals):
    ...
    # 2. contradiction checks (VERY IMPORTANT)

    if "failed" in text and trx.status == "completed":
        return EvidenceVerdict.inconsistent

    if "not sent" in text and trx.status == "completed":
        return EvidenceVerdict.inconsistent
